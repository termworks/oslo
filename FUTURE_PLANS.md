...
Forty candidates from three surveys (fish, zsh, nushell and friends; version control, jobs, remote
hosts and the editor seam; and the unusual shells: NGS, Oils/YSH, Hilbish, murex, Elvish, Eshell,
marcel, dgsh, ksh93, mksh, tcsh, rc, expect, IPython). Every one names the existing feature it
builds on, and none repeats something oslo already ships.

**Ranked by how much each would change daily use and how much nobody else has it.** The overview
is the short version; each idea's full section follows under [Details](#details).
...
* **`out`**: pipe the output of the command you already ran, without running it again.
* **`undo`**: put back what the last line took away.
* **`plan`**: what this line would actually do, every expansion and truncation, without running it.
* **`stuck`**: ask a job that has gone quiet what it is actually waiting for.
* **`norms`**: the shell knows how long this usually takes and how it usually ends.
* **`mute`**: mute a stage of the pipeline you are typing; it stays in the line and does not run.
* **`trace`**: `set -x`, except the trace comes back as rows you can filter.
* **`glob qualifiers`**: `*.log(older 7d, larger 1M)` becomes literal filenames on Tab.
...
| `try` | every exit code the pipeline produced, as rows | medium |
| `why` | the long version of the last error, and a hint | medium |
| `explain` | every word on the line labelled: which flag, what it consumes, what the redirect does | medium |
| `doc` | a key on any word shows the paragraph describing exactly that word, offline | small |
...
| `decode` | structured pipelines that start with an ordinary command, via a registered parser | medium |
| `derive` | your own computed column on `ls` or `ps`, once, in Lua | medium |
| `spread` | run once per row in parallel and get the results back as columns | medium |
| `heat` | a script's own source with elapsed time painted onto its lines | medium |
...
`branch history` · `unsaved` · `frames` · `origin` · `modifiers` · `trap check` · `bang` · `grab` ·
`bytes` · `collect` · `rewrite` · `hold` · `feed` · `offer`
